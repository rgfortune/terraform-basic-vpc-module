locals {
  ...
  public_subnet_cidrs = [for subnet in range(var.availability_zones_count) : cidrsubnet(var.cidr, 6, subnet)]
  ...
  private_subnet_cidrs = [for subnet in range(var.availability_zones_count, 25) : cidrsubnet(var.cidr, 6, subnet)]
